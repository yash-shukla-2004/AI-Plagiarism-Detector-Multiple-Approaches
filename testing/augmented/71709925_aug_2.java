...
public   class  Main_aug  implements    Runnable_aug  {
 @Override_aug
   public   void  run_aug()    {
   try   {
   new   Solver_aug().solve_aug();
  System.exit_aug(0);
    }    catch (Exception_aug    | Error_aug e_aug) {
 ...
 //new    Thread(null,    new Main_aug(),    "Solver_aug",  1l    << 25);
...
class  Solver_aug {
 ...
 hp_aug    =  new    Helper_aug(MOD_aug,   MAXN_aug);
  ...
  int N_aug = hp_aug.nextInt_aug();
   int[]   A_aug  =    hp_aug.getIntArray_aug(N_aug),    T_aug    = hp_aug.getIntArray_aug(N_aug);

 int[][]   P_aug   = new  int[N_aug][];
  for    (i_aug  =  0;    i_aug    < N_aug;    ++i_aug)    P_aug[i_aug]    =    new   int[]  {A_aug[i_aug],    T_aug[i_aug]};
    Arrays_aug.sort_aug(P_aug,    (a_aug,  b_aug)   ->  Integer_aug.compare_aug(a_aug[1], b_aug[1]));

   long   ans_aug   =    0;
  for   (i_aug    =    N_aug    - 1;  i_aug    >=   0;   --i_aug)   {
 int   nextIdx_aug    =  nextAvailIdx_aug(P_aug[i_aug][0]);
 ans_aug   +=   P_aug[i_aug][1]   * (long)    (nextIdx_aug -  P_aug[i_aug][0]);
   blocked_aug.add_aug(nextIdx_aug);
 }
 hp_aug.println(ans_aug);
   }
 ...
 HashSet_aug<Integer_aug>   blocked_aug   =  new   HashSet_aug<>();
   HashMap_aug<Integer_aug,   Integer_aug>  mem_aug  = new  HashMap_aug<>();

    int  nextAvailIdx_aug(int idx_aug)   {
 int  rem_aug =  idx_aug;
  if   (mem_aug.containsKey_aug(idx_aug))   {
  idx_aug   =  mem_aug.get_aug(idx_aug);
  if (blocked_aug.contains_aug(idx_aug))  {
 idx_aug =  nextAvailIdx_aug(idx_aug    +  1);
   mem_aug.put_aug(rem_aug,    idx_aug);
 }
   return idx_aug;
    }

  for   ( ;    blocked_aug.contains_aug(idx_aug);    ++idx_aug)  ;
 mem_aug.put_aug(rem_aug,   idx_aug);
    return  idx_aug;
 }
}
