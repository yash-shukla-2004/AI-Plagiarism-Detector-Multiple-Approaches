...
public  class  CF1288F_aug    extends  PrintWriter_aug   {
 ...
 public static void   main(String_aug[]  $)  {
  ...
  int[] ii_aug, jj_aug,   cc_aug,    ww_aug,    ww__aug;  int   m__aug;
  int[]   ae_aug,   dd_aug, kk_aug,   ff_aug; int  n__aug;
 ...
 void init_aug()   {
  ...
  ii_aug  =  new    int[m__aug];  jj_aug =    new   int[m__aug];
   cc_aug  =   new   int[m__aug  *  2];
    ww_aug    =    new   int[m__aug];    ww__aug   = new  int[m__aug];
   ae_aug    = new  int[n__aug];  dd_aug  = new   int[n__aug]; kk_aug    =    new int[n__aug];   ff_aug =  new int[n__aug];
 pq_aug  =    new    int[1  +   n__aug]; iq_aug = new   int[n__aug];
  m__aug  = 0;
   }
 void   link__aug(int  i_aug,  int  j_aug, int   c_aug,   int   w_aug) {
    int   h_aug    =    m__aug++;
    ii_aug[h_aug]   =   i_aug;  jj_aug[h_aug]    =    j_aug;   cc_aug[h_aug    <<   1]   = c_aug;    ww_aug[h_aug]    =    w_aug;
   ...
   return dd_aug[u_aug]   <    dd_aug[v_aug] ||  dd_aug[u_aug]   ==   dd_aug[v_aug]    &&  kk_aug[u_aug]    <    kk_aug[v_aug];
   }
 ...
 Arrays_aug.fill_aug(dd_aug,   INF_aug);
 dd_aug[s_aug] =   0;  pq_add_last_aug(s_aug);
   while   (cnt_aug  >    0) {
    int i_aug  = pq_remove_first_aug();
  int  k_aug   =   kk_aug[i_aug] +    1;
 for  (int  o_aug   =  ae_aug[i_aug];    o_aug    !=    0;  o_aug    =   oo_aug[o_aug])    {
    int    h__aug =    oh_aug[o_aug];
   if (cc_aug[h__aug]    > 0)   {
   int h_aug  = h__aug  >>  1;
 int j_aug =   i_aug    ^ ii_aug[h_aug]   ^  jj_aug[h_aug];
 int   d_aug    =   dd_aug[i_aug]  + ((h__aug &   1)   ==  0   ? ww__aug[h_aug]   :    -ww__aug[h_aug]);
    if    (dd_aug[j_aug]  >    d_aug   || (dd_aug[j_aug]  == d_aug    &&    kk_aug[j_aug]   >   k_aug))  {
    if    (dd_aug[j_aug]   == INF_aug)
 pq_add_last_aug(j_aug);
   dd_aug[j_aug]   =  d_aug; kk_aug[j_aug] =  k_aug;  ff_aug[j_aug] =  h__aug;
 pq_up_aug(j_aug);
 }
   }
 }
 }
    return    dd_aug[t_aug]  !=   INF_aug;
   }
    boolean    trace_aug(int   s_aug, int   t_aug) {
   int  sum_aug =   0;
    for (int   i_aug = t_aug; i_aug !=    s_aug;   )    {
    int    h__aug    =    ff_aug[i_aug],  h_aug    = h__aug    >>   1;
  sum_aug   +=  (h__aug    & 1)   ==    0    ? ww_aug[h_aug]    :    -ww_aug[h_aug];
 ...
 ww__aug[h_aug]  =    ww_aug[h_aug];
  while  (dijkstra_aug(s_aug,   t_aug))  {
    if (!trace_aug(s_aug,  t_aug))
  break;
  for   (int   h_aug    = 0;   h_aug < m__aug;  h_aug++) {
 int i_aug  =    ii_aug[h_aug],    j_aug = jj_aug[h_aug];
 if   (dd_aug[i_aug]  != INF_aug &&    dd_aug[j_aug]    !=    INF_aug)  {
    //    dd_aug[j_aug]    <=  dd_aug[i_aug]   +  ww__aug[h_aug]
   //  ww__aug[h_aug]    +   dd_aug[i_aug] -    dd_aug[j_aug]  >=    0$_aug
    ww__aug[h_aug] +=    dd_aug[i_aug] -   dd_aug[j_aug];
    }
  }
  }
    int  sum_aug   =   0;
  for    (int    h_aug  =    0;    h_aug    <    m__aug;    h_aug++)
  sum_aug    +=  ww_aug[h_aug]  *  cc_aug[h_aug    <<    1 | 1];
   return    sum_aug;
    }
  void    main() {
