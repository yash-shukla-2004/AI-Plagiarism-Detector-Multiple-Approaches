    ...
    void solve() throws Exception {
        ...
        for (int tce = 1; tce <= tc; tce++) {
            ...
            for (i = N - 1; i >= 0; --i) {
                double water = A[i];
                int count = 1;
                while (!right.isEmpty() && right.peek().level < water) {
                    Pair popped = right.pop();
                    double total = water * count + popped.level * popped.count;
                    count += popped.count;
                    water = total / count;
                }
                right.add(new Pair(water, count));
            }

            while (!right.isEmpty()) hp.println(right.pop());
        }
...
class Pair {
    ...
    public String toString() {
        ...
        while (count-- > 0) sb.append(level).append("\n");
        return sb.toString();
    }
}
