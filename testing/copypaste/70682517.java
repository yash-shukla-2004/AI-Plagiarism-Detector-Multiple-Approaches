...
class Solver {
    ...
    void solve() throws Exception {
        ...
        for (int tce = 1; tce <= tc; tce++) {
            int N = hp.nextInt();
            long[][] P = new long[N][2];
            for (i = 0; i < N; ++i) P[i] = hp.getLongArray(2);

            boolean ok = true;
            for (i = 0; i < N && ok; ++i) {
                j = (i + N / 2) % N;
                ok = check(P[i], P[(i + 1) % N], P[j], P[(j + 1) % N]);
            }
            hp.println(ok ? "YES" : "NO");
        }
    ...
    boolean check(long[] a1, long[] a2, long[] b1, long[] b2) {
        long dx1 = a1[0] - a2[0], dy1 = a1[1] - a2[1];
        long dx2 = b1[0] - b2[0], dy2 = b1[1] - b2[1];
