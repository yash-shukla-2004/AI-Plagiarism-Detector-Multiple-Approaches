...
class Solver {
    ...
    void solve() throws Exception {
        ...
        for (int tce = 1; tce <= tc; tce++) {
            int N = hp.nextInt();
            int[] A = hp.getIntArray(N), T = hp.getIntArray(N);

            int[][] P = new int[N][];
            for (i = 0; i < N; ++i) P[i] = new int[] {A[i], T[i]};
            Arrays.sort(P, (a, b) -> Integer.compare(a[1], b[1]));

            long ans = 0;
            for (i = N - 1; i >= 0; --i) {
                int nextIdx = nextAvailIdx(P[i][0]);
                ans += P[i][1] * (long) (nextIdx - P[i][0]);
                blocked.add(nextIdx);
            }
            hp.println(ans);
        }
    ...
    int nextAvailIdx(int idx) {
        int rem = idx;
        if (mem.containsKey(idx)) {
            idx = mem.get(idx);
            if (blocked.contains(idx)) {
                idx = nextAvailIdx(idx + 1);
                mem.put(rem, idx);
            }
            return idx;
        }

        for ( ; blocked.contains(idx); ++idx) ;
        mem.put(rem, idx);
        return idx;
    }
}
